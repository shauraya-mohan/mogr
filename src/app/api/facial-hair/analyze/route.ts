...
export async function POST(req: Request) {
  ...
  // Latest selfie for this user — the SAME photo the hair scan uses.
  ...
  const { data: existingProfile } = await supabase
    ...
    .select("growth, density, summary, scan_id, questionnaire")
    .eq("user_id", user.id)
    .maybeSingle();
  ...
  try {
    read = await visionJSON<VisionRead>({
      system: FACIAL_HAIR_SYSTEM_PROMPT,
      user: `Questionnaire answers: ${JSON.stringify(questionnaire)}. Read this man's facial hair / growth and recommend styles per the schema.`,
      imageDataUrl,
    });
  } catch (e) {
    return NextResponse.json(
  ...
  // Persist: shared read → profiles; facial-hair read → facial_hair_profiles.
  await supabase.from("profiles").update({ face_shape: read.face_shape }).eq("id", user.id);
  await supabase.from("facial_hair_profiles").upsert({
    user_id: user.id,
    scan_id: scan.id,
    growth: read.growth,
  ...
  await supabase.from("facial_hair_styles").delete().eq("user_id", user.id);
